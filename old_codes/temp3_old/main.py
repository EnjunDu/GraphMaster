...
def fix_messy_json(content):
    """
    A more robust function to extract and repair JSON from LLM outputs.
    """
    main_logger = logging.getLogger("main_logger")
    
    # Try multiple markers that might indicate the start of JSON data
    markers = [
    ...
    for marker in markers:
        ...
        if idx != -1:
            extracted_text = content[idx + len(marker):].strip()
            main_logger.info(f"[fix_messy_json] Found marker: '{marker}'")
            break
    
    # If we couldn't find any markers, log the first 100 chars of content for debugging
    if extracted_text == content:
        main_logger.warning(f"[fix_messy_json] No known markers found. Content starts with: {content[:100]}...")
    
    # Look for JSON-like patterns
    # First try to find array of objects pattern: [{...}]
    array_patterns = [
        r'\[\s*\{\s*"[^"]+"\s*:.*\}\s*\]',  # Standard JSON array
        r'\[\s*\{.*\}\s*\]',                # More lenient pattern
    ]
    ...
    for pattern in array_patterns:
        ...
        if match:
            ...
            main_logger.info(f"[fix_messy_json] Matched JSON array pattern")
            break
    else:
        # If no array pattern matched, try to find a single object pattern: {...}
        obj_match = re.search(r'\{\s*"[^"]+"\s*:.*\}', extracted_text, re.DOTALL)
        if obj_match:
            json_str = obj_match.group(0).strip()
            # Wrap it in an array for consistency
            json_str = f"[{json_str}]"
            main_logger.info(f"[fix_messy_json] Matched single JSON object pattern and wrapped in array")
        else:
            main_logger.error("[fix_messy_json] Failed to match any JSON pattern")
            return []
    ...
    try:
        ...
        main_logger.info(f"[fix_messy_json] Successfully parsed JSON with {len(json_data)} items")
        return json_data
    except json.JSONDecodeError as e:
        main_logger.error(f"[fix_messy_json] JSON parse error: {e}")
        
        # Attempt aggressive repair by removing problematic sections
        try:
            # Remove unescaped control characters
            json_str = re.sub(r'[\x00-\x1F\x7F]', '', json_str)
            # Fix quotes issues (unmatched quotes)
            json_str = re.sub(r'(?<!")(\w+)(?=":)', r'"\1"', json_str)
            # Replace invalid escape sequences
            json_str = re.sub(r'\\([^"\\/bfnrtu])', r'\1', json_str)
            
            json_data = json.loads(json_str)
            main_logger.info(f"[fix_messy_json] Successfully parsed JSON after repairs")
            return json_data
        except Exception as repair_e:
            main_logger.error(f"[fix_messy_json] Failed to repair JSON: {repair_e}")
            main_logger.error(f"[fix_messy_json] Problematic JSON snippet: {json_str[:200]}...")
            return []
